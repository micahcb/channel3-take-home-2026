...
Assumptions that scale: structured extraction (category → product) and most products don't change often so we can run periodic recrawls. Assumptions that don't: reading the whole catalog on every request, and having no search index. High llm costs per run

**Frontend: API for agentic shopping and developer tools**

To power agentic shopping apps we'd expose an API centered on search and discovery: e.g. `GET /api/search?q=...&brand=...&category=...&limit=...` returning the same product schema that we have built, plus `GET /api/products/{id}` for full details. Agents need a single entry point to "find products" and "get one product" which is the core API and then you could add extensions: a recommendations endpoint (e.g. "similar to this product" or "often bought with") and a structured output or tool schema (e.g. OpenAPI/JSON Schema) so agents can call search and product endpoints as tools. For developers we'd provide: (1) an API key and usage dashboard for rate limits and cost visibility, (2) SDKs that wrap search and product calls, (3 )A playground with example agent flows (e.g. "search → compare prices → return top 3") so new shopping experiences can be built without re-implementing the pipeline.